...
async function importCosmosProducts() {
  try {
    ...
    for (const categoryName of categories) {
      const existingCategory = await sql`SELECT id FROM categories WHERE name = ${categoryName}`
      ...
      if (existingCategory.length > 0) {
        ...
        const newCategory = await sql`
          ...
          VALUES (
            ...
            ${"COSMOS certified " + categoryName.toLowerCase() + " products"}
          )
          RETURNING id
        `
        categoryMap[categoryName] = newCategory[0].id
    ...
    for (const body of certificationBodies) {
      ...
      const existingCert = await sql`SELECT id FROM certifications WHERE name = ${certName}`

      if (existingCert.length > 0) {
        certificationMap[body] = existingCert[0].id
        console.log(`[v0] Found existing certification: ${certName}`)
      } else {
        const newCert = await sql`
          INSERT INTO certifications (name, category, description, website_url)
          VALUES (
            ${certName},
            ${"Organic/Natural"},
            ${"COSMOS certification by " + body + " - ensuring organic and natural cosmetic standards"},
            ${body.includes("ECOCERT") ? "https://www.ecocert.com" : "https://www.soilassociation.org"}
          )
          RETURNING id
        `
        certificationMap[body] = newCert[0].id
    ...
    const existingVendor = await sql`SELECT id FROM vendors WHERE business_name = 'COSMOS Certified Products'`
    ...
    if (existingVendor.length > 0) {
      ...
      const newVendor = await sql`
        INSERT INTO vendors (business_name, business_description, business_email, is_verified, commission_rate)
        VALUES (
    ...
    for (let i = 0; i < products.length; i += batchSize) {
      ...
      console.log(`[v0] Processing batch ${Math.floor(i / batchSize) + 1}/${Math.ceil(products.length / batchSize)}`)
        ...
        try {
          ...
          const insertedProduct = await sql`
            INSERT INTO products (
              name, brand, description, price, compare_at_price, sku, 
              vendor_id, category_id, is_active, inventory_quantity, notes
            )
            VALUES (
              ${product["Commercial name"]},
              ${product["Brand name"]},
              ${product["COSMOS Signature"] + " certified " + category.toLowerCase() + " by " + product["Brand name"] + ". Certified by " + product["Certified by"] + " under COSMOS version " + product["Version"] + "."},
              ${price},
              ...
              ${"Company: " + product["Company name"] + " | COSMOS: " + product["COSMOS Signature"] + " | Version: " + product["Version"]}
            )
            RETURNING id, name
          `
          ...
          if (certificationId) {
            await sql`
              INSERT INTO entity_certifications (
                entity_id, entity_type, certification_id, verified, certification_data
              )
              VALUES (
          ...
          insertedCount++
          console.log(`[v0] Inserted: ${insertedProduct[0].name}`)
        } catch (error) {
